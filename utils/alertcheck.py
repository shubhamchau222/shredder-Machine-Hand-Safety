...
def drawboxtosafeline(image_np,p1,p2,Line_Position2,Orientation):
    ...
    if(Orientation=="bt"):
        ...
        if(bounding_mid):
            cv2.line(img=image_np, pt1=bounding_mid, pt2=(bounding_mid[0],Line_Position2), color=(255, 255, 0), thickness=1, lineType=8, shift=0)
    ...
    if (distance_from_line <= 0) :
         ...
         cv2.putText(image_np, "ALERT", (posii, 50),cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255,0), 2)
         #sound = os.path.join()
         playsound(r"D:\DLCVNLP\Projects\shredderMachine\shredder-machine-hosur\utils\final_alarm.wav")
